...
export const fetchMentorIndustries = createAsyncThunk(
    ...
    async (_, { getState, rejectWithValue }) => {
        ...
        try {
            ...
            return rejectWithValue(err.response?.data?.message || "Failed to fetch industries");
        }
    }
);
...
export const fetchMentorList = createAsyncThunk(
    ...
    async ({ page = 1, limit = 6 } = {}, { getState, rejectWithValue }) => {
        ...
        if (mentorList.length > 0 && Date.now() - mentorListFetchedAt < STALE_AFTER_MS) {
            return null; // still fresh
        }
        try {
            const res = await axiosInstance.get(`/mentors?page=${page}&limit=${limit}`);
            return res.data.data;
        } catch (err) {
...
const referenceDataSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addMatcher(
                (action) => action.type.startsWith("referenceData/") && action.type.endsWith("/pending"),
                (state) => { state.loading = true; state.error = null; }
            )
            .addMatcher(
                (action) => action.type.startsWith("referenceData/") && action.type.endsWith("/rejected"),
                (state, action) => { state.loading = false; state.error = action.payload; }
            );
    },
});

export const { invalidateMentorList, invalidateIndustries } = referenceDataSlice.actions;
