...
const HandleModal = ({
  ...
  const handleRefund = async () => {
    try {
      ...
      await adminAxiosInstance.post(`/admin/reports/${report.id}/refund`, {
        adminNote,
      });
      onRefund(report.id);
      onClose();
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            >
              <div className="flex items-center justify-between">
                <p
                  className="text-[10px] font-700 uppercase tracking-widest text-slate-400"
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={label}>
                    <p
                      className="text-[9px] font-700 uppercase tracking-widest text-slate-400"
                      style={{ fontWeight: 700 }}
                    >
                      {label}
                    </p>
                    <p
                      className="text-xs font-600 text-slate-700 mt-0.5"
              ...
              {report.totalAmount > 0 && (
                ...
                >
                  ...
                  <p
                    className="text-sm font-700 text-slate-800 mt-0.5"
              ...
              {report.description && (
                ...
                >
                  <p
                    className="text-[9px] font-700 uppercase tracking-widest text-slate-400 mb-1"
                    style={{ fontWeight: 700 }}
                  >
                    Description
                  </p>
            ...
            {isRefundReport && (
              <div
                className="p-4 rounded-2xl space-y-3"
                style={{
                  background: alreadyRefunded ? "#f0fdf4" : "#fffbeb",
                  border: `1px solid ${alreadyRefunded ? "#bbf7d0" : "#fde68a"}`,
                }}
              >
                <div className="flex items-center gap-2">
                  ...
                  <p
                    ...
                    style={{
                      fontWeight: 700,
                      color: alreadyRefunded ? "#059669" : "#d97706",
                    }}
                  >
                    {alreadyRefunded
                ...
                {!alreadyRefunded && (
                  <>
                    <p className="text-xs text-slate-600 leading-relaxed">
                      This will refund{" "}
                      <strong>{report.totalAmount} tokens</strong> from escrow
                      back to the mentee's wallet and close the session.
                    </p>
                    <button
                      type="button"
            ...
            {hasSession && (
              ...
              >
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Permanently deletes this connect request. Both mentee and
            ...
            <div>
              <p
                className="text-xs font-600 text-slate-500 mb-2"
                style={{ fontWeight: 600 }}
              >
                Update Status{" "}
                <span className="text-slate-400 font-400">(select one)</span>
              </p>
              <div className="grid grid-cols-2 gap-3">
                {STATUSES.map((s) => {
              ...
              >
                ...
                <p className="text-[10px] text-blue-700 leading-relaxed">
                  The reporter will receive a notification when you save
                  changes.
                </p>
              </div>
            </div>
            ...
            <div>
              <label
                className="text-xs font-600 text-slate-500 block mb-1.5"
                style={{ fontWeight: 600 }}
              >
                Admin Note{" "}
                <span className="text-slate-400 font-400">(optional)</span>
              </label>
              <textarea
                value={adminNote}
            ...
            {error && (
              <p
                className="text-xs text-red-500 font-600"
                style={{ fontWeight: 600 }}
              >
                {error}
              </p>
...
const AdminReports = () => {
  ...
  const handleSave = (id, updated) => {
    setReports((prev) =>
      prev.map((r) =>
        r.id.toString() === id.toString() ? { ...r, ...updated } : r,
      ),
    );
    fetchStats();
    ...
    setReports((prev) =>
      prev.map((r) =>
        r.id.toString() === id.toString()
          ? { ...r, status: "resolved", refundProcessed: true }
          : r,
      ),
    );
    fetchStats();
  ...
  const handleDeleteSession = (id) => {
    setReports((prev) =>
      prev.map((r) =>
        r.id.toString() === id.toString()
          ? { ...r, status: "resolved", connectRequestId: null }
          : r,
      ),
    );
    fetchStats();
                  ...
                  reports.map((report) => (
                    ...
                    >
                      <td className="px-5 py-4">
                        <p
                          className="text-xs font-600 text-slate-800"
                          style={{ fontWeight: 600 }}
                        >
                          {report.mentee || "—"}
                        </p>
