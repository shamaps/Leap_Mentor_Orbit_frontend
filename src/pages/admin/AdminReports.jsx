...
const StatusBadge = ({ status }) => {
  const cfg = SEPARATE_STATUS_MAP[status] || SEPARATE_STATUS_MAP.open;
  return (
    <span
...
const CategoryBadge = ({ category }) => {
  const cfg = UNIQUE_CAT_MAP[category] || UNIQUE_CAT_MAP.other;
  return (
    <span
...
const HandleModal = ({ report, onClose, onSave, onRefund, onDeleteSession }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      const res = await adminAxiosInstance.patch(`/admin/reports/${report.id}`, { status: selectedStatus, adminNote });
  ...
  const handleRefund = async () => {
    try {
      ...
      await adminAxiosInstance.post(`/admin/reports/${report.id}/refund`, { adminNote });
      onRefund(report.id);
      onClose();
    } catch (err) {
  ...
  const handleDeleteSession = async () => {
    try {
      setActionLoading(true);
      await adminAxiosInstance.delete(`/admin/reports/${report.id}/session`);
...
const AdminReports = () => {
  ...
  const fetchReports = useCallback(async (targetPage = 1, currentQuery = search, activeStatus = statusFilter) => {
    try {
      ...
      const res = await adminAxiosInstance.get("/admin/reports", { params: queryParams });
