...
const isOnlyNumbers = (val: string) => Boolean(val && /^\d+$/.test(val.trim()));
...
export function getFieldErrorMap(schema: z.ZodTypeAny, form: unknown) {
...
// change either.
export function getFirstErrorMessage(schema: z.ZodTypeAny, form: unknown) {
    const result = schema.safeParse(form);
