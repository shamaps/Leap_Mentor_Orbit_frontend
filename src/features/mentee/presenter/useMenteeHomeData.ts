...
import { mapMentorCard } from "@/features/mentor/model/mentorMapper";

// Element type of the mentors array searchMentors resolves to — kept as
// a derived type rather than re-declared, same convention as useMentorSearch.ts.
export type MentorCardShape = NonNullable<ReturnType<typeof mapMentorCard>>;

// Only the fields this hook actually reads off the mentee profile.
interface MenteeHomeProfile {
    skills?: string[];
    interestedFields?: string[];
}

// ── Fetches recommended mentors + upcoming sessions + wallet balance 
export const useMenteeHomeData = (profile: MenteeHomeProfile | null | undefined) => {
    const [mentors, setMentors] = useState<MentorCardShape[]>([]);
    const [loadingMentors, setLoadingMentors] = useState(true);

    const [sessions, setSessions] = useState<MappedConnectRequest[]>([]);
        ...
        const fetchSessions = async () => {
            try {
                ...
                const upcoming = (data.requests || [])
                    .filter((r): r is MappedConnectRequest => r !== null && (r.status === "accepted" || r.status === "ongoing"))
                    .sort((a, b) => {
