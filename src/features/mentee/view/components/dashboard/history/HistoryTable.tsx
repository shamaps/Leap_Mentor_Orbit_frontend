...
import type { ConnectRequestSummary } from "@/features/mentee/presenter/useRequestHistory";
...
const ViewButton = ({ onClick, isSelected }: { onClick: () => void; isSelected: boolean }) => (
  <button
    type="button"
    onClick={onClick}
    className={`flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full border transition-all ${isSelected
...
const getGradient = (name = "") =>
  AVATAR_GRADIENTS[(name.codePointAt(0) ?? 0) % AVATAR_GRADIENTS.length];

interface HistoryTableProps {
  requests: ConnectRequestSummary[];
  selected: ConnectRequestSummary | null;
  onSelect: (request: ConnectRequestSummary | null) => void;
...
const HistoryTable = ({ requests, selected, onSelect, onDelete }: HistoryTableProps) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Cards */}
      {requests.map((request: ConnectRequestSummary) => {
        const mentor = request.mentor as { name?: string } | undefined;
        const mentorProfile = request.mentorProfile as { currentRole?: string } | undefined;
        const role = mentorProfile?.currentRole || "—";
        ...
        return (
          <div
            key={request._id}
            className={`bg-white rounded-2xl border shadow-sm px-5 py-4 grid grid-cols-[2.5fr_1.5fr_1.2fr_1fr_1fr] gap-4 items-center transition-all ${isSelected
