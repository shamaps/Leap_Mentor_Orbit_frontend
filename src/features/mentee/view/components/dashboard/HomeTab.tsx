...
import { useMenteeHomeData, type MentorCardShape } from "@/features/mentee/presenter/useMenteeHomeData";
import { useLeapPointsRequest } from "@/features/mentee/presenter/useLeapPointsRequest.js";
import type { MappedConnectRequest, ConnectSlot } from "@/features/connects/model/connectRequestMapper";
// ── Helpers ───────────────────────────────────────────────────
...
const formatSlotDate = (slot: ConnectSlot | null | undefined) => {
  if (!slot?.date) return "";
...
const formatSlotTime = (slot: ConnectSlot | null | undefined) => {
...
const calculateProfileCompletion = (profile: Record<string, unknown> | null | undefined) => {
  if (!profile) return 0;
  const communicationPreferences = profile.communicationPreferences as string[] | undefined;
  ...
  const fields = [
    ...
    (communicationPreferences?.length ?? 0) > 0,
...
interface MentorCardComponentProps {
  mentor: MentorCardShape;
  onViewProfile: (mentor: MentorCardShape) => void;
}

const MentorCard = ({ mentor, onViewProfile }: MentorCardComponentProps) => {
...
interface SessionCardProps {
  request: MappedConnectRequest;
  index: number;
  navigate: NavigateFunction;
}

const SessionCard = ({ request, index, navigate }: SessionCardProps) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 px-3 py-2.5 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow">
      <div
        className={`w-9 h-11 rounded-xl flex flex-col items-center justify-center text-white ${getAccent(index)} shrink-0`}
      >
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="flex items-center gap-1.5">
          ...
          <StatusBadge status={isOngoing ? "ongoing" : "accepted"} variant="history" />
        </div>
...
// ── Leap Points Panel ─────────────────────────────────────────
const LeapPointsPanel = ({ balance, loading }: { balance: number; loading: boolean }) => {
...
const HomeTab = () => {
  ...
  const [selectedMentor, setSelectedMentor] = useState<MentorCardShape | null>(null);
  ...
  if (loadingMentors) {
    ...
    mentorGridContent = mentors.map((mentor) => (
      <MentorCard
  ...
  if (loadingSessions) {
    ...
    sessionsListContent = sessions.map((request, idx) => (
      <SessionCard
        key={request._id}
        request={request}
        index={idx}
      ...
      <div className="flex flex-col gap-6 -mt-2">
        ...
        <div className="flex items-center justify-between gap-6 flex-wrap">
          ...
          {completionPct < 100 && (
            <button
              type="button"
              className="flex flex-col items-center gap-1 shrink-0 cursor-pointer hover:opacity-80 transition-opacity bg-transparent border-none"
              onClick={() =>
                globalThis.dispatchEvent(
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              {(profile?.skills as string[] | undefined)?.[0] && (
                <p className="text-xs text-slate-600 mt-0.5">
                  Based on your skill:{" "}
                  <span className="font-semibold text-blue-900">
                    {(profile?.skills as string[])[0]}
                  </span>
                </p>
              )}
            </div>
