...
const MentorProfileModal = ({
  ...
  const toggleSlot = async (slot: PublicTimeSlot, group: PublicSpecificDate) => {
    setLockError("");
                      ...
                      {availableGroups.map((group, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-[11px] font-bold leading-tight ${isCurrentActive ? "text-white" : "text-slate-600"}`}>{(group.displayDate || group.day || group.date).split(",")[0]}</span>
                            <span className={`text-[9px] font-medium mt-0.5 ${isCurrentActive ? "text-blue-100" : "text-slate-400"}`}>{(group.displayDate || "").split(", ")[1] || group.date}</span>
                          </button>
                        );
                      })}
                    </div>
                    ...
                    {activeGroup && (
                      <div>
                        <div className="flex items-center justify-between mb-2.5">
                          <span className="text-xs font-bold text-slate-600">{activeGroup.displayDate || activeGroup.day || activeGroup.date}</span>
