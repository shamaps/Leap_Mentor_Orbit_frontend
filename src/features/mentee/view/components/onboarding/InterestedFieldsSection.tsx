// components/mentee/onboarding/InterestedFieldsSection.tsx
import { useState, forwardRef, type ChangeEvent, type KeyboardEvent } from "react";
...
const TagInput = ({ id, tags, onAdd, onRemove, placeholder, error }: TagInputProps) => {
...
const InterestedFieldsSection = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const removeFromArray = (field: string, value: string) => {
    handleChange({
      target: {
        name: field,
        value: (form[field] || []).filter((v: string) => v !== value),
      },
    });
  };
