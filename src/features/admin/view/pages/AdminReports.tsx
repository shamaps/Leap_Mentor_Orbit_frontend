...
import { useHandleReportModal } from "../../presenter/useHandleReportModal";
...
const StatusBadge = ({ status }: { status: string }) => {
  const cfg = SEPARATE_STATUS_MAP[status as keyof typeof SEPARATE_STATUS_MAP] || SEPARATE_STATUS_MAP.open;
  return (
    <span
...
const CategoryBadge = ({ category }: { category: string }) => {
  const cfg = UNIQUE_CAT_MAP[category as keyof typeof UNIQUE_CAT_MAP] || UNIQUE_CAT_MAP.other;
  return (
    <span
...
// ── Confirm Dialog ─────────────────────────────────────────────
const ConfirmDialog = ({ title, message, confirmLabel, confirmColor, onConfirm, onCancel, loading }: { title: string; message: string; confirmLabel: string; confirmColor: string; onConfirm: () => void; onCancel: () => void; loading: boolean }) => (
  <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 px-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4" style={{ fontFamily: FONT }}>
      ...
      <div className="flex gap-3">
        <button
          onClick={onCancel} disabled={loading} className="flex-1 py-2.5 rounded-xl text-xs font-600 transition-all"
          style={{ background: "#f1f5f9", color: "#475569", fontWeight: 600, border: "1px solid #e2e8f0" }}
        >
          Cancel
        </button>
        <button
          onClick={onConfirm} disabled={loading} className="flex-1 py-2.5 rounded-xl text-xs font-600 text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2"
...
// ── Handle Modal ───────────────────────────────────────────────
const REPORT_RESOLUTION_STATUSES = [
...
const HandleModal = ({ report, onClose, onSave, onRefund, onDeleteSession }: { report: any; onClose: () => void; onSave: (id: string, updated: any) => void; onRefund: (id: string) => void; onDeleteSession: (id: string) => void }) => {
  const {
    ...
    handleRefund,
    handleDeleteSession,
  } = useHandleReportModal(report, { onSave, onRefund, onDeleteSession, onClose });
  ...
  const metaProfileSummary = [
    { keyName: "Mentee", valText: report.mentee },
    { keyName: "Mentor", valText: report.mentor },
    { keyName: "Reported By", valText: report.reportedBy },
    { keyName: "Date", valText: report.date }
  ];

  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col" style={{ fontFamily: FONT, maxHeight: "90vh" }}>
          <div className="flex items-center justify-between px-6 py-5 border-b flex-shrink-0" style={{ borderColor: "#e8eaf0" }}>
            <div>
            ...
            <button
              type="button" onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center transition-all" style={{ background: "#f1f5f9" }}
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            <div className="p-4 rounded-2xl space-y-3" style={{ background: "#f8fafc", border: "1px solid #e8eaf0" }}>
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-700 uppercase tracking-widest text-slate-400" style={{ fontWeight: 700, letterSpacing: "0.1em" }}>Report Summary</p>
                <CategoryBadge category={report.category} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                {metaProfileSummary.map(({ keyName, valText }) => (
                  <div key={keyName}>
                    <p className="text-[9px] font-700 uppercase tracking-widest text-slate-400" style={{ fontWeight: 700 }}>{keyName}</p>
                    <p className="text-xs font-600 text-slate-700 mt-0.5" style={{ fontWeight: 600 }}>{valText || "—"}</p>
                  </div>
                ))}
              </div>
              ...
              {report.screenshotUrl && (
                <div className="pt-2">
                  <a href={report.screenshotUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline flex items-center gap-1">
            ...
            {isRefundReport && (
              <div className="p-4 rounded-2xl space-y-3" style={{ background: alreadyRefunded ? "#f0fdf4" : "#fffbeb", border: `1px solid ${alreadyRefunded ? "#bbf7d0" : "#fde68a"}` }}>
                <div className="flex items-center gap-2">
                  <span style={{ fontSize: 13, fontWeight: 800, fontFamily: MONO, color: "currentColor", letterSpacing: "-0.02em" }}>LP</span>
                ...
                {!alreadyRefunded && (
                  <>
                    <p className="text-xs text-slate-600 leading-relaxed">This will refund <strong>{report.totalAmount} tokens</strong> from escrow back to the mentee&apos;s wallet and close the session.</p>
                    <button type="button" onClick={() => setConfirmRefund(true)} disabled={!hasSession || report.paymentStatus !== "paid"} className="w-full py-2.5 rounded-xl text-xs font-600 text-white transition-all disabled:opacity-40" style={{ background: "#d97706", fontWeight: 600 }}>
                      Process Refund — {report.totalAmount} tokens
                    </button>
                    {report.paymentStatus !== "paid" && <p className="text-[10px] text-slate-400">No payment found for this session.</p>}
                  </>
                )}
              </div>
            )}
            ...
            {hasSession && (
              <div className="p-4 rounded-2xl space-y-3" style={{ background: "#fef2f2", border: "1px solid #fecaca" }}>
                <div className="flex items-center gap-2">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /></svg>
                  <p className="text-xs font-700 text-red-600" style={{ fontWeight: 700 }}>Delete Session</p>
                </div>
                <p className="text-xs text-slate-600 leading-relaxed">Permanently deletes this connect request. Both mentee and mentor will be notified.</p>
                <button type="button" onClick={() => setConfirmDelete(true)} className="w-full py-2.5 rounded-xl text-xs font-600 text-white transition-all" style={{ background: "#dc2626", fontWeight: 600 }}>
                  Delete This Session
                </button>
              </div>
            )}
            ...
            <div>
              <p className="text-xs font-600 text-slate-500 mb-2" style={{ fontWeight: 600 }}>Update Status <span className="text-slate-400 font-400">(select one)</span></p>
              <div className="grid grid-cols-2 gap-3">
                {REPORT_RESOLUTION_STATUSES.map((s) => {
                  const currentSelected = selectedStatus === s.key;
                  return (
                    <button
                      key={s.key} type="button" onClick={() => setSelectedStatus(s.key)} className="py-3 rounded-xl text-sm font-600 transition-all flex items-center justify-center gap-2"
                      style={{ fontWeight: 600, background: currentSelected ? s.color : "#f8fafc", color: currentSelected ? "white" : "#64748b", border: currentSelected ? `2px solid ${s.color}` : "2px solid #e2e8f0" }}
                    >
                      {currentSelected && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>}
                      {s.label}
                    </button>
                  );
                })}
              ...
              <div className="mt-3 flex items-start gap-2 px-3 py-2.5 rounded-xl" style={{ background: "#eff6ff", border: "1px solid #bfdbfe" }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="2" className="shrink-0 mt-0.5"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
                <p className="text-[10px] text-blue-700 leading-relaxed">The reporter will receive a notification when you save changes.</p>
              </div>
            </div>
          ...
          <div className="flex gap-3 px-6 py-4 border-t flex-shrink-0" style={{ borderColor: "#e8eaf0" }}>
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl text-xs font-600 transition-all" style={{ background: "#f1f5f9", color: "#475569", fontWeight: 600, border: "1px solid #e2e8f0" }}>Cancel</button>
            <button type="button" onClick={handleSave} disabled={saving} className="flex-1 py-2.5 rounded-xl text-xs font-600 text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: "#2563eb", fontWeight: 600 }}>
      ...
      {confirmRefund && (
        <ConfirmDialog
          title="Process Refund?" message={`This will refund ${report.totalAmount} tokens from escrow back to the mentee's wallet and close the session. This cannot be undone.`}
          confirmLabel="Yes, Process Refund" confirmColor="#d97706" loading={actionLoading} onConfirm={handleRefund} onCancel={() => setConfirmRefund(false)}
        />
      )}

      {confirmDelete && (
        <ConfirmDialog
          title="Delete Session?" message="This will permanently delete the connect request. Both mentee and mentor will be notified. This cannot be undone."
          confirmLabel="Yes, Delete Session" confirmColor="#dc2626" loading={actionLoading} onConfirm={handleDeleteSession} onCancel={() => setConfirmDelete(false)}
        />
      )}
    </>
  );
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-700 text-slate-800" style={{ fontWeight: 700, fontFamily: FONT }}>Reports &amp; Complaints</h1>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid #e8eaf0" }}>
          <div className="px-6 py-4 border-b space-y-3" style={{ borderColor: "#e8eaf0" }}>
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="relative">
                ...
                <input
                  value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Search mentee or mentor..." className="pl-8 pr-3 py-1.5 rounded-xl text-xs outline-none transition-all"
            ...
            <div className="flex gap-1.5 flex-wrap">
              {STATUS_FILTERS.map(({ key, label }) => (
                <button
                  key={key} onClick={() => handleStatusFilter(key)} className="px-3 py-1.5 rounded-xl text-xs font-600 transition-all"
            ...
            <table className="w-full" style={{ fontFamily: FONT }}>
              <thead>
                <tr style={{ background: "#f1f5f9", borderBottom: "2px solid #e2e8f0" }}>
                  {DATA_GRID_HEADERS.map((headerText) => (
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: "#e8eaf0" }}>
            <p className="text-xs text-slate-400" style={{ fontFamily: MONO }}>Showing {reports.length} of {pagination.totalCount} reports</p>
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchReports(pagination.currentPage - 1)} disabled={pagination.currentPage <= 1} className="px-3 py-1.5 rounded-xl text-xs font-600 transition-all border border-e2e8f0 bg-f1f5f9 text-475569 disabled:opacity-30"
              >
                Previous
              </button>
              <span className="text-xs text-slate-400" style={{ fontFamily: MONO }}>{pagination.currentPage} / {pagination.totalPages || 1}</span>
              <button
                onClick={() => fetchReports(pagination.currentPage + 1)} disabled={pagination.currentPage >= pagination.totalPages} className="px-3 py-1.5 rounded-xl text-xs font-600 text-white bg-2563eb transition-all disabled:opacity-30"
              >
                Next
              </button>
            </div>
      ...
      {selected && (
        <HandleModal
          report={selected} onClose={() => setSelected(null)} onSave={handleSave} onRefund={handleRefund} onDeleteSession={handleDeleteSession}
        />
      )}
    </>
  );
