...
export const useAdminUserManagement = () => {
  ...
  // ── Toast ─────────────────────────────────────────────────
  const showToast = ({ message, type = "success" }: { message: string; type?: string }) => {
  ...
  const fetchUsers = useCallback(
    async (page = 1, q = search, role = roleFilter, blocked = showBlocked) => {
      try {
        setLoading(true);
        const params: { page: number; limit: number; search?: string; role?: string; deleted?: boolean } = { page, limit: 15 };
