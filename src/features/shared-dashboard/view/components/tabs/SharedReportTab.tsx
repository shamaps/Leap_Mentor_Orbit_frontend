...
// ── Star Rating Input ─────────────────────────────────────────
const StarRatingInput = ({ value, onChange, disabled, labelledBy }: { value: number; onChange: (v: number) => void; disabled?: boolean; labelledBy?: string }) => (
  <div className="flex items-center gap-1.5" role="radiogroup" aria-labelledby={labelledBy}>
    {[1, 2, 3, 4, 5].map((star) => (
      <button
        key={star}
        type="button"
...
// ── Feedback Card ─────────────────────────────────────────────
const FeedbackCard = ({ feedback, label, isOwn }: { feedback: Feedback; label: string; isOwn: boolean }) => (
  <div
...
// ── Main ──────────────────────────────────────────────────────
const SharedReportTab = ({ connect, reportRefreshKey }: { connect?: SharedConnect | null; reportRefreshKey?: number }) => {
