// src/components/shared-dashboard/tabs/SharedNotesTab.tsx
...
import { selectConnect, selectAuthUser } from "@/app/store/selectors";
...
const FILE_MAP: Record<string, { label: string; bg: string; text: string; border: string; icon: string }> = {
...
const UploadModal = ({ onUpload, uploading, onClose, isPrivateView }: UploadModalProps) => {
  ...
  const performFileValidation = (file?: File | null) => {
    setFileError("");
    if (!file) return;
...
const NoteCard = ({ note, myId, onDelete, isPrivateView = false }: NoteCardProps) => {
  ...
  const uploadedById = typeof uploadedBy === "string" ? uploadedBy : uploadedBy?._id;
  const activeUserMatch = uploadedById === myId;
...
const SharedFilesSection = ({ connect }: { connect: SharedConnect | null | undefined }) => {
  ...
  const processCollection = () => {
    const sequence: Array<{ type: "separator" | "note"; note?: SharedNote; dateStr?: string; key: string }> = [];
    ...
    sharedFilesContent = (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full" style={{ gridAutoRows: "1fr" }}>
        {processCollection().map((item) => {
          ...
          return <NoteCard key={item.key} note={item.note!} myId={myId} onDelete={async (id) => { await deleteNote(id, false); }} isPrivateView={false} />;
        })}
      </div>
    );
  }
