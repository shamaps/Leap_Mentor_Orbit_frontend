...
interface GoalCardProps {
  ...
  saving: boolean;
  onAdd: (goalId: string, fields: { title: string; dueDate?: string }) => Promise<ActionResult>;
  onToggle: (milestoneId: string, isCompleted: boolean) => Promise<void>;
  onDelete: (milestoneId: string) => Promise<void>;
}

const GoalCard = ({ goal, onEdit, milestones, saving, onAdd, onToggle, onDelete }: GoalCardProps) => {
...
const OverallProgress = ({ completedSlots, totalSlots, progress, onLeaveFeedback, feedbackSubmitted }: OverallProgressProps) => {
...
// ── Main ──────────────────────────────────────────────────────
const SharedGoalsTab = ({ onAllComplete }: { onAllComplete?: () => void }) => {
  const connect = useSelector(selectConnect);
  const [isEditing, setIsEditing] = useState(false);
  const [showFeedbackModal, setShowFeedbackModal] = useState(false);
  const [feedbackSlotIndex, setFeedbackSlotIndex] = useState<number | undefined>(undefined);
  const [showOverallFeedbackModal, setShowOverallFeedbackModal] = useState(false);
  const handleSessionComplete = (slotIndex: number) => {
  ...
  const handleCreateGoal = async (fields: { title: string; description?: string; startDate?: string; endDate?: string }) => {
    if (!connectRequestId) return;
    const result = await createGoal(fields);
    if (result?.success) setIsEditing(false);
  };
  const handleUpdateGoal = async (goalId: string, fields: Record<string, unknown>) => {
  ...
  if (isEditing) {
    goalSectionContent = (
      <GoalForm
        initial={goal || {}}
        onSave={goal ? (fields) => handleUpdateGoal(goal._id, fields as unknown as Record<string, unknown>) : handleCreateGoal}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {showFeedbackModal && (
        <FeedbackModal
          connect={connect as ModelSharedConnect | null}
