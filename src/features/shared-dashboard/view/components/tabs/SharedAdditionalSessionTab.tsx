...
const compileDateLabel = (dateStr: string) => {
...
const ConfirmModal = ({ slot, onConfirm, onCancel, saving }: { slot: Slot; onConfirm: () => void; onCancel: () => void; saving: boolean }) => {
...
const SuccessScreen = ({ slot, onDone }: { slot: Slot; onDone: () => void }) => {
  return (
...
const AdditionalSessionPaymentModal = ({ connect, slot, slotId, onClose, onSuccess }: { connect?: SharedConnect | null; slot?: Slot | null; slotId: string; onClose: () => void; onSuccess: () => void }) => {
  ...
  const sessionRate = Number(connect?.mentorProfile?.hourlyRate) || 0;
  ...
  const triggerEscrowPayment = async () => {
    ...
    if (insufficient) {
      setError(`Need ${totalAmount - (walletBalance ?? 0)} more tokens.`);
      return;
    }
    ...
    try {
      ...
      setError(responseMessage || message || "Payment failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const metadataRows: { name: string; detail: string }[] = [
  ...
  ].filter((row): row is { name: string; detail: string } => Boolean(row));
...
const SharedAdditionalSessionTab = ({ onTabChange }: { onTabChange?: (tab: string) => void }) => {
  ...
  const availableGroups = availability.filter((g: PublicSpecificDate) =>
    g.slots.some((s: PublicTimeSlot) => !existingSlotDates.some((e) => e.date === g.date && e.startTime === s.startTime))
  );
  const activeGroup = availableGroups[activeDayIndex] || null;

  const totalAvailable = availableGroups.reduce(
    (acc: number, g: PublicSpecificDate) => acc + g.slots.filter((s: PublicTimeSlot) => !existingSlotDates.some((e) => e.date === g.date && e.startTime === s.startTime)).length, 0
  );

  const isSlotBooked = (date: string, startTime: string) => existingSlotDates.some((e) => e.date === date && e.startTime === startTime);

  const toggleSlot = (slot: PublicTimeSlot, group: PublicSpecificDate) => {
    setConfirmSlot({
                ...
                {!availLoading && !availError && availableGroups.length > 0 && (
                  <>
                    <div className="flex gap-2">
                      {availableGroups.map((group: PublicSpecificDate, idx: number) => {
                    ...
                    {activeGroup && (
                      <div>
                        <div className="flex items-center justify-between mb-2.5">
                          ...
                          <span className="text-[10px] text-slate-400 font-medium">{activeGroup.slots.filter((s: PublicTimeSlot) => !isSlotBooked(activeGroup.date, s.startTime)).length} open</span>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {activeGroup.slots.map((slot: PublicTimeSlot) => (
