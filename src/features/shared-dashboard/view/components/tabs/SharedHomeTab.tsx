...
const assignBackgroundStyle = (seedName = "") => {
  ...
  const targetIndex = (seedName.codePointAt(0) ?? 0) % customPalette.length;
...
const formatSlot = (timeSlot?: SessionSlot | null): string | null => {
  if (!timeSlot) return null;
  const renderedDate = new Date(`${timeSlot.date}T00:00:00`).toLocaleDateString("en-US", {
    weekday: "long", month: "long", day: "numeric", year: "numeric",
  });

  const parseHourString = (timeVal?: string): string => {
...
const PersonCard = ({
  ...
  name: string;
  profile?: { profilePicture?: string; currentRole?: string; company?: string; skills?: string[] } | null;
  roleLabel: string;
}) => {
...
const InfoRow = ({ icon, label, value, accent }: { icon: ReactNode; label: string; value: ReactNode; accent?: string }) => (
...
const SharedHomeTab = ({ slots = [], onTabChange = () => { } }: { slots?: SessionSlot[]; onTabChange?: (tab: string) => void }) => {
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4">
          ...
          {slots.length > 0
            ...
            : Boolean(confirmedSlot) && <InfoRow icon={CALENDAR_VECTOR} label="Confirmed Session" value={formatSlot(confirmedSlot as SessionSlot)} />}
          {totalAmount != null && (
            <InfoRow
