...
const getGradient = (name = "") =>
  GRADIENTS[(name.codePointAt(0) ?? 0) % GRADIENTS.length];
...
const Avatar = ({ name, picture, size = 32 }: AvatarProps) => {
  if (picture) {
    return (
      <img
        src={picture}
...
// ── Read Receipt ──────────────────────────────────────────────
const ReadReceipt = ({ readAt }: { readAt?: string | null }) => (
...
// ── Empty State ───────────────────────────────────────────────
const EmptyState = ({ otherName }: { otherName?: string }) => (
  <div
    style={{
      flex: 1,
      display: "flex",
