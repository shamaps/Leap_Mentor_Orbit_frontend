 ...
 */
export const AuthMessageBanner = ({ type, text }: { type?: BannerType; text?: string }) => {
  if (!text) return null;
  ...
  return (
    <div
      className={`rounded-lg px-4 py-2.5 text-sm mb-4 leading-relaxed ${classes[type as BannerType] || classes.error}`}
    >
      {text}
    </div>
  );
