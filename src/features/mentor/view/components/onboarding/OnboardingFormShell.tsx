...
import { MentorOnboardingFormContext, type MentorOnboardingChangeEvent } from "@/features/mentor/context/MentorOnboardingFormContext";
import {
  selectAuthToken,
...
const OnboardingFormShell = () => {
  ...
  const loading = useAppSelector(selectMentorOnboardingLoading);
  const error = useAppSelector(selectMentorOnboardingError);
  const successMsg = useAppSelector(selectMentorOnboardingSuccessMsg);
  ...
  const [form, setForm] = useState(() => {
    const saved = sessionStore.getJSON<typeof EMPTY_MENTOR_FORM>("mentorOnboardingForm");
  ...
  const sectionRefs: Record<string, RefObject<HTMLDivElement | null> | undefined> = {
