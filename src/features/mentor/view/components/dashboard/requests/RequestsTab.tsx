...
const RequestsTab = () => {
  ...
  const requests = useAppSelector(selectIncomingRequests);
  const loading = useAppSelector(selectConnectRequestsLoading);
  ...
  const getTabBadgeClass = (isActive: boolean, tabKey: string) => {
          ...
          <div className="flex overflow-x-auto scrollbar-none">
            {TABS.map((tab) => (
              <button
                ...
                className={`shrink-0 sm:flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs sm:text-sm font-semibold transition-all duration-150 border-b-2 whitespace-nowrap ${activeTab === tab.key
