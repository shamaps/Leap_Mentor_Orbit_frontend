...
const getOverlappingBusy = (dateStr: string, slot: TimeSlot, busySlots: BusySlot[]): BusySlot[] => {
...
const removeSlotById = (day: SpecificDate, slotId: string): SpecificDate => ({
  ...day,
...
const getSlotError = (startTime: string, endTime: string, minDuration: number): string | null => {
...
const getEventsForDate = (dateStr: string, events: CalendarEventItem[]): CalendarEventItem[] => {
...
const parse24 = (timeStr?: string): TimeParts => {
...
const format24 = ({ hour12, minute, period }: TimeParts): string => {
  let h = hour12 % 12;
  if (period === "PM") h += 12;
