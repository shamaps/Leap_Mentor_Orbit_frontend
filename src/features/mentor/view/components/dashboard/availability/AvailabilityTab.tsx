// components/mentor/dashboard/availability/AvailabilityTab.tsx
...
import type { BusySlot, SpecificDate } from "@/features/mentor/model/availability.types";
...
const collectBusyConflicts = (specificDates: SpecificDate[], busySlots: BusySlot[]): BusyConflict[] => {
...
const AvailabilityTab = () => {
  ...
  const [pendingConflicts, setPendingConflicts] = useState<BusyConflict[]>([]);
  // ── NEW: track whether all calendar slots pass validation ──────────────────
  const [isAvailabilityValid, setIsAvailabilityValid] = useState<boolean>(true);
  ...
  return (
    <div className="space-y-5">
      ...
      {msg.text && (
        <div
          className={`flex items-center gap-2 text-sm font-medium rounded-xl px-4 py-3 border ${msg.type === "success"
