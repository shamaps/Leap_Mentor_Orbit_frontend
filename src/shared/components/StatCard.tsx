...
type StatCardVariant = "admin" | "simple" | "home" | "earnings";
...
const StatCard = ({
  ...
  if (variant === "simple") {
    return (
      <div
        className={`flex items-center gap-3 bg-white rounded-2xl border px-4 py-3.5 flex-1 min-w-0 ${accent ? "border-blue-200 bg-blue-50/40" : "border-slate-100"}`}
      >
        <div
          className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 ${accent ? "bg-blue-100" : "bg-slate-100"}`}
        >
          {icon}
        </div>
        <div className="min-w-0">
          ...
          <p
            className={`text-[10px] sm:text-xs font-semibold mt-1 leading-tight ${accent ? "text-blue-600" : "text-slate-500"}`}
          >
            {label}
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-slate-700 mt-1" style={{ fontWeight: 500 }}>
          {label}
        </p>
