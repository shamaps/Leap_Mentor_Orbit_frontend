...
export default function HelpCenter() {
  ...
  const handleSupportDispatch = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setErrorBannerMsg("");
    try {
      await submitSupportMessage({ ...ticketForm, role: selectedRole });
  ...
  const SUPPORT_FORM_SCHEMAS: { id: keyof typeof ticketForm; mode: string; inputType?: string; placeholderText: string; rowsCount?: number }[] = [
        ...
        {formSubmitted ? (
          ...
          <form onSubmit={handleSupportDispatch} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {SUPPORT_FORM_SCHEMAS.map((field) => {
              ...
              const handleInputFocus = (e: FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => { e.target.style.borderColor = COLOR_CONFIG.primary; };
              const handleInputBlur = (e: FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => { e.target.style.borderColor = "#e2e8f0"; };
