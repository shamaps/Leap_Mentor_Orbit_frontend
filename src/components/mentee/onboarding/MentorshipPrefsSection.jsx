...
const MentorshipPrefsSection = () => {
  ...
  const knownLanguages = form.languages || [];
  ...
  const syncFormPreference = (inputName, nextStateArray) => {
    handleChange({
      target: { name: inputName, value: nextStateArray }
    });
  };

  const processChannelToggle = (targetKey) => {
  ...
  const processLanguageToggle = (targetLang) => {
              ...
              {COMM_OPTIONS.map(({ value, label, icon }) => {
                ...
                return (
                  <label key={value} className="flex items-center gap-3 cursor-pointer group">
                    <div
                      onClick={() => processChannelToggle(value)}
                    ...
                    <span onClick={() => processChannelToggle(value)} className="text-sm text-slate-600 select-none">
                      {icon} {label}
                    </span>
                  </label>
                );
