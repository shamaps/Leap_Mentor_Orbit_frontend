...
const computeByteSizeLabel = (byteCount) => {
  if (!byteCount) return "—";
...
const formatCustomDateString = (isoTimestamp) => {
  if (!isoTimestamp) return "";
  const dateObj = new Date(isoTimestamp);
  return dateObj.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
};

const generateDateHeader = (targetTimestamp) => {
...
const PrivateFileCard = ({ note, onDelete }) => {
  ...
  const cfg = ENUMERATED_FILE_MESSAGES[note.fileType] || ENUMERATED_FILE_MESSAGES.other;
...
PrivateFilesSection.propTypes = {
  connect: PropTypes.shape({ _id: PropTypes.string, status: PropTypes.string }),
};
