      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {uploading ? <><Spinner size="sm" light />Uploading...</>
               : (
                <>
                  <svg
                    width="14"
...
const NoteCard = ({ note, myId, onDelete, isPrivateView = false }) => {
  ...
  const handleDownload = async () => {
    try {
      ...
      anchor.href = url;
      anchor.download = note.fileName || "download";
...
const SharedFilesSection = ({ connect }) => {
  ...
  const { notes, loading, uploading, error, uploadNote, deleteNote } = useNotes(
    connect?._id,
  );
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {notes.length} file{notes.length !== 1 ? "s" : ""} shared with your
            session partner
          </p>
        </div>
        {!isCompleted && (
